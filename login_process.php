...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (validateLogin($username, $password)) {
        ...
        // If login fails, redirect back to the login page with an error flag and message
        $errorMessage = "Invalid username or password.";
        header("Location: login.php?error=1&message=" . urlencode($errorMessage)); // Redirect to login page with an error flag and message
        exit();
    }
} else {
